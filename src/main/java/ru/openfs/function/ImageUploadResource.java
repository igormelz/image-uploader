...
public class ImageUploadResource {
        ...
        public Response uploadImage(MultipartFormDataInput body) throws Exception {
                ...
                try {
                        ...
                        return Response.status(Response.Status.GATEWAY_TIMEOUT)
                                        .entity("Service unavailable. Try again later.").build();
                }
                ...
                // store to db
                Map<String, String> uids = db.query(Request.newBuilder().addMutations(setImage(bucket, object, title))
                ...
                minio.putObject(PutObjectArgs.builder().bucket(bucket).object(object).tags(uids)
                                .contentType(fileInput.getMediaType().getType() + "/"
                                                + fileInput.getMediaType().getSubtype())
                                .stream(fileInput.getBody(InputStream.class, null), -1, 5 * 1024 * 1024).build());
                body.close();

                // call thumbnail
                String thumbnailJson = String.format(
                                "{ \"bucket\":\"%s\", \"object\":\"%s\", \"imageUid\":\"%s\", \"imageSizeUid\": \"%s\"}",
                                bucket, object, uids.get("image"), uids.get("size"));
                var response = client.send(
                                HttpRequest.newBuilder(thumbnailUrl).header("Content-type", "application/json")
                                                .POST(BodyPublishers.ofString(thumbnailJson)).build(),
        ...
        public void deleteImage(@PathParam("id") String id) throws Exception {
                ...
                String pathJson = db.query(Request.newBuilder()
                                .setQuery(String.format("{dd(func: uid(%s)) { Image.sizes { ImageSize.path }}}", id))
                                .setReadOnly(true).build()).getJson().toStringUtf8();
                ...
                db.query(Request.newBuilder()
                                ...
                                .addMutations(Mutation.newBuilder()
                                                .setDelNquads(ByteString.copyFromUtf8("uid(v) * * .\n")).build())
                                .setCommitNow(true).build());
                // remove id
                db.query(Request.newBuilder().addMutations(Mutation.newBuilder()
        ...
        private Mutation setImage(String bucket, String object, String title) {
                ...
                setNQuads.add(NQuad.newBuilder().setSubject("_:image").setPredicate("Image.date")
                                .setObjectValue(Value.newBuilder()
                                                .setStrVal(ZonedDateTime.now().format(DateTimeFormatter.ISO_INSTANT))
                                                .build())
                                .build());
                setNQuads.add(NQuad.newBuilder().setSubject("_:image").setPredicate("Image.dt")
                                .setObjectValue(Value.newBuilder().setIntVal(System.currentTimeMillis() / 1000).build())
                                .build());
                setNQuads.add(NQuad.newBuilder().setSubject("_:image").setPredicate("Image.sizes").setObjectId("_:size")
                                .build());
        ...
        private String decodeTitle(String title) {
                ...
                return decodedTitle.contains(".") ? decodedTitle.substring(0, decodedTitle.lastIndexOf("."))
                                : decodedTitle;
        }
}